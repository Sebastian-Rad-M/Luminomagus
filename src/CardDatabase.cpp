...
void CardDatabase::loadAllCards() {
	...
	showcaseOfKnowlege.addEffect(std::make_unique<LambdaEffect>([](RoundTracker& state) {
		state.addScore(state.getHand().getCards().size() * 3);}));
	...
	library["c_showcase_of_knowlege"] = std::move(showcaseOfKnowlege);
			...
			discernFromDarkness.addEffect(std::make_unique<LambdaEffect>([](RoundTracker& state) {
				...
				for (int p = 0; p < actualKeep; p++) {
					std::string prompt="";
					for (size_t i = 0; i < revealed.size(); i++) prompt += "  [" + std::to_string(i + 1) + "] " + revealed[i]->getName() + "\n";
					prompt += "  Select card to keep: ";    int index = ui->requestChoice(prompt, 1, revealed.size())- 1;
	...
	// Uncommons
			...
			[](RoundTracker& state) {
				auto ui = state.getUI();
				int choice = ui->requestChoice("\n  --- INTERFERE WITH THE PATTERN ---\n  [1] Deck\n  [2] Hand\n  [3] Graveyard\n  [4] Exile\n  Select a zone to shuffle:", 1, 4);
				switch(choice) {
					case 1: 
						state.getDeck().shuffle(); 
						ui->showMessage("  --> Deck shuffled."); 
						break;
					case 2: 
						state.getHand().shuffle(); 
						ui->showMessage("  --> Hand shuffled? what are you doing man"); 
						break;
					case 3: 
						state.getGraveyard().shuffle(); 
						ui->showMessage("  --> Graveyard shuffled."); 
						break;
					case 4: 
						state.getExile().shuffle(); 
						ui->showMessage("  --> Exile shuffled."); 
						break;}}));
	...
	// (Adjust the cost arguments to match your exact Card constructor order!)
		...
		adNauseum.addEffect(std::make_unique<LambdaEffect>([](RoundTracker& state) {
            ...
            while (true) {
                ...
                ui->showMessage("  --> Revealed: " + topCard->getName() 
                        + " (Cost: " + std::to_string(cardCost) + "). Total so far: " 
                        + std::to_string(totalCostRevealed) + "/13");

                if (totalCostRevealed > 13) {
                    ui->showMessage("  [!!!] Total cost exceeded 13 (" + std::to_string(totalCostRevealed) + "). YOU LOSE.");
                    state.addScoreToTarget(9999);
                    ui->showMessage("Please concede here");
                    state.triggerLoss();
                    break;
                }
		...
		internalReflection.addEffect(std::make_unique<LambdaEffect>([](RoundTracker& state) {
				...
				int x = ui->requestChoice("Choose an X(lower than" + std::to_string(maxX) + ")", 0, maxX);
				for (int i = 0; i < x; i++) {
					auto& handCards = hand.getCards();
					std::string prompt = "";
					for (size_t j = 0; j < handCards.size(); j++) {
						prompt += "  [" + std::to_string(j + 1) + "] " + handCards[j]->getName() + "\n";
					}
					int choice = ui->requestChoice(prompt, 1, handCards.size());
					int index = choice - 1;

					ui->showMessage("  --> Discarded " + handCards[index]->getName());
		...
		radiantDecay.addEffect(std::make_unique<LambdaEffect>([](RoundTracker& state) {
			...
			std::string prompt = "";
			for (size_t i = 0; i < graveCards.size(); i++) prompt += "  [" + std::to_string(i + 1) + "] " + graveCards[i]->getName() + "\n";
			prompt += "  Select card to decay: ";
			int index = ui->requestChoice(prompt, 1, graveCards.size()) - 1;
		...
		doomsday.addEffect(std::make_unique<LambdaEffect>([](RoundTracker& state) {
			...
			for (int p = 0; p < cardsToPick; p++) {
				std::string prompt = "\nPick " + std::to_string(p + 1) + " of " + std::to_string(cardsToPick) + ") ---\n";
				for (size_t i = 0; i < purgatory.size(); i++) prompt += "  [" + std::to_string(i + 1) + "] " + purgatory[i]->getName() + "\n";
				int choice = ui->requestChoice(prompt, 1, purgatory.size());
				int index = choice - 1;
			...
			ui->showMessage("\n  --> " + std::to_string(exiledCount) + " remaining cards have been EXILED.");
	...
	// 2 Generic, 1 Green, Yags will
		...
		dreamingOfThePast.addEffect(std::make_unique<LambdaEffect>([](RoundTracker& state) {
				auto& grave = state.getGraveyard();
				auto& hand = state.getHand();
	...
	// 2 Generic, 1 Red, Rare
		...
		lookUponMyWorks.addEffect(std::make_unique<LambdaEffect>([](RoundTracker& state) {
				auto ui = state.getUI();
				ui->showMessage("  --> You may pay multiples of 2 Light to score 20 points per multiple.");
				int x = ui->requestChoice("  How many multiples of 2? ", 0, 2000);
		...
		digTroughMemories.addEffect(std::make_unique<LambdaEffect>([](RoundTracker& state) {
			...
			if (maxExile == 0) {
				ui->showMessage("  [!] Graveyard is empty. You exile 0, look at 0.");
				return; 
			}
            int numToExile = ui->requestChoice("  Graveyard has " + std::to_string(maxExile) + " cards.\n  How many do you want to exile from the top? (0 - " + std::to_string(maxExile) + "): ", 0, maxExile);
			...
			for (int p = 0; p < picks; p++) {
                std::string prompt = "\n  --- MEMORY DREDGE (Pick " + std::to_string(p + 1) + "/" + std::to_string(picks) + ") ---\n";
				for (size_t i = 0; i < revealed.size(); i++) prompt += "  [" + std::to_string(i + 1) + "] " + revealed[i]->getName() + "\n";
                prompt += "  Select card to keep: ";
                int index = ui->requestChoice(prompt, 1, revealed.size()) - 1;
				hand.addCard(revealed[index]);
                ui->showMessage("  --> Added " + revealed[index]->getName());
				revealed.erase(revealed.begin() + index);
			}
			if (!revealed.empty()) for (auto& leftover : revealed) deck.addCardToBottom(leftover); 
			}));
